...
namespace SpdReaderWriter {
	...
	public class Device {
		...
		public Device(string PortName, int eepromAddress, int SpdSize) {
			this.PortName = PortName;
			this.EepromAddress = eepromAddress;
			...
			lock (device.PortLock) {
				if (device.IsConnected) {
					...
					while (device.BytesToRead < 1) {
						...
						// This is inside the loop, because it takes a few tries to get the device to start responding if it hasn't been accessed before it was plugged in.
