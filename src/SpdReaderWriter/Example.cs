...
namespace SpdReaderWriterConsole {
	class Example {
		...
		public static void Example_BasicUse() {
			...
			Eeprom.ReadByte(myDevice, 0, (int)SpdSize.DDR4_SPD_SIZE);
		...
		public static void Example_TestRealDevice() {
			...
			// Test a real device
			Device MyReader = new Device("COM3", 0x50, SpdSize.DDR4_SPD_SIZE);
			MyReader.Test(); //true
		}

		public static void Example_DuplicateRam() {
			// Copy SPD contents from one DIMM to another
			Device source = new Device("COM3", 80, SpdSize.DDR4_SPD_SIZE);
			Device destination = new Device("COM4", 82, source.SpdSize);

			for (int i = 0; i < (int)source.SpdSize; i++) {
		...
		public static void Example_FixCRC() {
			...
			Device MyReader = new Device("COM3", 0x52, SpdSize.DDR4_SPD_SIZE);

			// Read first 126 bytes
			byte[] spdHeader = Eeprom.ReadByte(MyReader, 0, 126);

			// Calculate CRC
			ushort crc = Eeprom.Crc16(spdHeader);
		...
		public static void Example_EraseSPD() {
			...
			Device MyReader = new Device("COM3", 0x50, SpdSize.DDR4_SPD_SIZE);
			for (int i = 0; i <= (int)MyReader.SpdSize; i++) {
